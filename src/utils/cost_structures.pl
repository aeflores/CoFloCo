...
compress_cost_structures(Cs_list,Head,Inv,New_cost):-
    ...
    append(Loops,Loops1),
    cost_structure_simplify_it_vars(cost(Exp1,Loops1,Norms3),New_cost).         
...
cost_structure_simplify(cost(Exp,Loops,Norms),Ctx,Cost):-
		       ...
		       cost_structure_simplify_it_vars(cost(Exp2,Loops3,Norms2),Cost).
...
cost_structure_simplify_it_vars(cost(Exp,[],Norms),cost(Exp,[],Norms)):-!.     		      
cost_structure_simplify_it_vars(cost(Exp,_,_),cost(Exp,[],[])):-
          Exp==inf,!.   		      

cost_structure_simplify_it_vars(cost(Exp,Loops,Norms),Cost):-	            
		       ...
		       Cost=cost(Exp,Resulting_loops_set,Norms2_set)
		       ).
...
infinite_loop(Loop_pairs):-
  member(([],loop(_,Exp,_,_)),Loop_pairs),
  number(Exp),
  Exp > 0.
...
get_loop_hash(loop(It_var,Exp,InternalLoops,Norms),(Hash,loop(It_var,Exp,InternalLoops,Norms))):-
	copy_term((Exp,InternalLoops,Norms),Term),
	numbervars(Term,0,_),
	term_hash(Term,Hash).
...
annotate_loop(N_norms,loop(It_var,Exp,InternalLoops,Norms),(Set,loop(It_var,Exp,InternalLoops,Norms))):-
