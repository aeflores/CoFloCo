...
%    (Bottoms\=[]-> format(user_error,'We could eliminate ~p loops in ~p~n',[N_bot,[Entry_phase]]);true),
...
%    include(nad_is_bottom,Cs_list,Bottoms),
%        length(Bottoms,N_bot),
%    (Bottoms\=[]-> format(user_error,'We could eliminate ~p loops in ~p~n',[N_bot,[Phase|Chain]]);true),
    nad_list_lub(Cs_list,Inv_out),
    Entry_call2=Entry_Call,
   (nad_is_bottom(Inv_out)->
