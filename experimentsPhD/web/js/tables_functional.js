...
function get_source_file_ref(name){
    name = name.split(" ")[0];
    return Source_dir+name+Source_ending;

}
function get_crs_file_ref(name){
    name = name.split(" ")[0];
    return crs_dir+name+crs_ending;
...
function file_column(name){
  ...
  if(type_bound=="upper_fun"){
  var sourceFile_ces=get_crs_file_ref(name);
  div.appendChild(createLink(sourceFile_ces," (abs_file)"));
  }
  col.appendChild(div);
  return col;
}
 ...
 function result_column(system,name,result,type_cell){
  var name_file = name.split(" ")[0];
  var source=results_dir[system]+name_file+out_extension[system];
