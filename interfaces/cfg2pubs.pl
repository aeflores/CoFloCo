...
save_exec:-
	...
	qsave_program('cfg2pubs',[stand_alone(true),goal(main_bin),foreign(save)]),
...
cfg2pubs([]).
cfg2pubs([F|Fs]) :-
	catch(cfg2pubs_1(F),Error,(format(user_error,'~p~n',[Error]),halt)),
	!,
	cfg2pubs(Fs).
...
transform_in_node(Node,Dest,C,cons(Head_vars,Call_vars,Cs)):-
	...
	assertz(eq(Head,C,[Call_dest,Loop_cont],Cs)),!.
...
who_calls_who(Loops,Loop):-
	include(cfg_edge_aux(Loop),Loops,Called),
	include(cfg_edge_rev(Loop),Loops,Caller),
	format('~p calls ~p and is called by ~p ~n',[Loop,Called,Caller]).
